...
{
    public class FramesCopy : StoryboardObjectGenerator
    {
        public override void Generate()
        {
		    var layer = GetLayer("7frameCopy");
            ...
            while(l < loops){
                for(int i = 0; i < sprites.Count; i ++){
                    ...
                    if(i == 1){
                        ...
                        sprites[i].MoveY(OsbEasing.OutElastic, start + (beat * i), start + (beat * i) + 250, 00, 5);
                    }
                    ...
                    for(int j = 0; j < moreMove; j ++){
                        sprites[i].MoveX(OsbEasing.InBack, start + (beat * (i + j)), 
